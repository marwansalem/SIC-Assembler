...
public static boolean simpleExpression(Instruction ins){
   ...
   //}else if(){ 
       ...
       return true;
        ...
        ins.setExpression1(operand.trim().toLowerCase().trim());
        ins.setExpression2("");
    }
    return false;
}
...
public static void findErrors(Instruction ins){
    if(OperationMap.getPair(ins.getMnemonic().trim().toLowerCase()) ==null){
        ins.appendErrorMessage("\t\t****Error: Unknown or Unsupported Instruction");
        return;
    }
    int form = OperationMap.OperationToFormat.get(ins.getMnemonic().trim().toLowerCase()).getFormat();
    /*if(form ==OperationMap.NullGuard.getFormat()) {
    ...
    }else if(  form>5 ||form<1  ){ // non register
        memoryErrors(ins);
